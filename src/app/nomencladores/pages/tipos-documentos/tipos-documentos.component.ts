...
export class TiposDocumentosComponent {
  ...
  filtrarDatos() {
    this.dataSource.data = this.tiposDocumentos.filter((tipoDocument) => {
      return (
        tipoDocument.descripcion
          .toLowerCase()
  ...
  cargarTiposDocumentos() {
    this.nomencladoresService
      ...
      .subscribe((tiposDocumentos) => {
        ...
        this.dataSource.data = tiposDocumentos;
  ...
  checkboxLabel(row?: TipoDocumento): string {
    if (!row) {
    ...
    if (id !== undefined) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.nomencladoresService.deleteTipoDocumento(id).subscribe(
            () => {
