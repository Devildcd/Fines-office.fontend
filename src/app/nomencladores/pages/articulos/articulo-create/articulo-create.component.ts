...
export class ArticuloCreateComponent {
  ...
  crearArticulo() {
    ...
    this.nomencladoresService.postArticulo(this.articulo).subscribe(
      ( articuloCreado) => {
        this.articulo =  articuloCreado;
        Swal.fire({
          icon: 'success',
          title: '¡Éxito!',
        ...
        setTimeout(() => {
          this.router.navigate(['/nomencladores/divisiones/detalles/articulo/:id', this.articulo.id]);
        }, 1000);
      }
      // error => {
