...
@Component({
  ...
  styleUrls: ['./tipos-operaciones.component.css'],
})
export class TiposOperacionesComponent {
  ...
  selection = new SelectionModel<TipoOperacion>(true, []);
  loading = true;
  ...
  filtrarDatos() {
    this.dataSource.data = this.tiposOperaciones.filter((tipoOperacion) => {
      return (
  ...
  cargarTiposOperaciones() {
    this.nomencladoresService
      ...
      .subscribe((tiposOperaciones) => {
        this.tiposOperaciones = tiposOperaciones;
        this.dataSource.data = tiposOperaciones;
  ...
  checkboxLabel(row?: TipoOperacion): string {
    if (!row) {
  ...
  eliminarTipoOperacion(id: number) {
    if (id !== undefined) {
      Swal.fire({
        title: '¿Estás seguro?',
        ...
        if (result.isConfirmed) {
          this.nomencladoresService.deleteTipoOperacion(id).subscribe(
            () => {
              // Eliminar sin recargar la página
              this.cargarTiposOperaciones();
              console.log('Tipo de Operación eliminada exitosamente');
            }
            // (error) => {
            //   console.error(error);
            //   if (error?.error?.message === 'Unauthenticated.') {
