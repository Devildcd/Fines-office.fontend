...
export class CreateComponent {
  ...
  crearEntregaRecepcionImposicion() {
    ...
    // };
    this.entregarecepcionimposicionService.postEntrega_Recepcion_Imposicion(formattedData).subscribe(
      (entregarecepcionCreado) => {
        this.entregas_recepcion_imposicion = entregarecepcionCreado;
        Swal.fire({
          icon: 'success',
          title: '¡Éxito!',
        ...
        setTimeout(() => {
          this.router.navigate(['/talonarios/entrega-recepcion-imposicion/detalles', this.entregas_recepcion_imposicion.id]);
        }, 1000);
      }
      // error => {
